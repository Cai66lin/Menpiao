...
public class Postman {
    ...
    public void init() throws Exception {
        ...
        //设置发件人的账户邮箱和授权码
        transport.connect("601309417@qq.com", "vuexmzsswpuvbfii");
    }
    ...
    private MimeMessage formatAdmissionTicket(AdmissionTicket ticket) throws Exception {
        ...
        message.setFrom(new InternetAddress("601309417@qq.com"));
        message.setRecipients(Message.RecipientType.TO, ticket.getEmail());
        message.setSubject("请查收门票凭证");
        MimeBodyPart text = new MimeBodyPart();
        text.setContent("<p>请查收门票凭证</p>", "text/html;charset=UTF-8");
        MimeBodyPart attachment = new MimeBodyPart();
        byte[] data = readPdf(ticket.getOrderID() + ".pdf");
        DataHandler dataHandler = new DataHandler(new ByteArrayDataSource(data, "application/pdf"));
        attachment.setDataHandler(dataHandler);
        // 设置附件的文件名（需要编码）
        attachment.setFileName(MimeUtility.encodeText("门票凭证.pdf"));
